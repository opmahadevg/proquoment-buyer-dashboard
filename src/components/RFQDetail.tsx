...
export default function RFQDetail({ rfq, onClose }: RFQDetailProps) {
  ...
  useEffect(() => {
    const fetchMatchedQuotes = async () => {
      ...
      try {
        const { data, error } = await supabase.from('quotes').select('*').eq('rfq_id', rfq.id);
        if (!error && data) {
          setQuotes(data);
        }
      } catch (err) {
            ...
            {loadingQuotes ? (
              ...
              <div className="space-y-3">
                {quotes.map((quote) => (
                  <div
                    key={quote.id}
                    className="border border-gray-100 rounded-xl p-4 bg-white shadow-sm hover:border-primary/20 transition-all"
                  >
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        ...
                        <div className="text-xs font-semibold text-gray-800">
                          Unit Price:{' '}
                          <span className="text-emerald-600">${quote.landed_cost_per_unit}</span>
                        </div>
                      </div>
                      <span
