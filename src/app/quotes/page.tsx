      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
        <div className="mb-6">
          ...
          <p className="text-sm text-[var(--muted-foreground)] mt-1">
            Review landed-cost quotes from Admin and accept or request revisions
          </p>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          <div className="bg-white border border-[var(--border)] rounded-xl p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-xs text-[var(--muted-foreground)]">Pending Review</p>
          ...
          <div className="bg-white border border-[var(--border)] rounded-xl p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-xs text-[var(--muted-foreground)]">Accepted</p>
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-sm text-[var(--muted-foreground)]">
              No quotes received yet. Submit an RFQ to get started!
            </p>
          </div>
        ) : (
          <div className="space-y-4">
            ...
            {sent.length > 0 && (
              <>
                <h2 className="text-sm font-bold text-[var(--foreground)] flex items-center gap-2">
                  <Clock size={14} /> Awaiting Your Decision
                </h2>
                {sent.map((q) => (
                  <div
                    key={q.id}
                    className="bg-white border-2 border-blue-200 rounded-2xl p-5 shadow-sm"
                  >
                    ...
                    <div className="bg-gray-50 rounded-xl p-4 mb-4">
                      <h3 className="text-xs font-bold text-[var(--muted-foreground)] uppercase mb-3">
                        Landed Cost Breakdown
                      </h3>
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                        <div>
                          <p className="text-[10px] text-[var(--muted-foreground)]">UNIT PRICE</p>
                          <p className="text-sm font-bold font-mono">
                            ${q.supplierUnitPrice?.toFixed(2)}
                          </p>
                        </div>
                        <div>
                          <p className="text-[10px] text-[var(--muted-foreground)]">QC FEE</p>
                          <p className="text-sm font-mono">${q.qcFee?.toFixed(2)}</p>
                        </div>
                        <div>
                          <p className="text-[10px] text-[var(--muted-foreground)]">FREIGHT</p>
                          <p className="text-sm font-mono">${q.freightCost?.toFixed(2)}</p>
                        </div>
                        <div>
                          <p className="text-[10px] text-[var(--muted-foreground)]">INSURANCE</p>
                          <p className="text-sm font-mono">${q.insurance?.toFixed(2)}</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-end justify-between">
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          className="flex items-center gap-1.5 px-4 py-2 border border-red-200 text-red-600 text-sm font-semibold rounded-lg hover:bg-red-50 transition-colors"
                        >
                          <X size={14} /> Reject
                        </button>
                        <button
            ...
            {[...accepted, ...rejected].length > 0 && (
              <>
                <h2 className="text-sm font-bold text-[var(--foreground)] mt-6 flex items-center gap-2">
                  <FileText size={14} /> Past Quotes
                </h2>
                {[...accepted, ...rejected].map((q) => (
                  <div
                    key={q.id}
                    className="bg-white border border-[var(--border)] rounded-xl p-4 opacity-80"
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <span className="font-mono text-xs font-semibold">{q.id}</span>
                        <span
                          className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${q.status === 'accepted' ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-600'}`}
                        >
                      ...
                      <div className="text-right">
                        <p className="text-sm font-bold font-mono">
                          ${q.landedCostPerUnit?.toFixed(2)}/unit
                        </p>
                        <p className="text-xs text-[var(--muted-foreground)]">
                          Total: ${q.totalValue?.toLocaleString()}
                        </p>
                      </div>
                    </div>
                  </div>
