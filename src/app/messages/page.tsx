...
const QUICK_TEMPLATES = [
  { label: 'Request Update', text: 'Could you please provide an update on the current status of my order?' },
  { label: 'Delivery Query', text: 'I wanted to check on the expected delivery timeline for my shipment.' },
  { label: 'Quote Clarification', text: 'I have some questions regarding the latest quote. Could we schedule a call?' },
  { label: 'Document Request', text: 'Please send me the shipping documents and export paperwork for my order.' },
];
    ...
    <AppLayout>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
        ...
        <div className="mb-6 flex items-center justify-between">
          <div>
            <p className="text-xs font-medium text-[var(--muted-foreground)] uppercase tracking-wide mb-1">Communication</p>
        ...
        <div className="flex gap-4" style={{ height: 'calc(100vh - 220px)', minHeight: 500 }}>
          ...
          <div className="w-80 flex-shrink-0 bg-white border border-[var(--border)] rounded-2xl overflow-hidden flex flex-col">
            <div className="px-4 py-3 border-b border-[var(--border)] flex items-center gap-2">
              ...
              {unread > 0 && (
                <span className="ml-auto px-2 py-0.5 bg-primary text-white text-[10px] font-bold rounded-full">{unread}</span>
              )}
            </div>
            <div className="flex-1 overflow-y-auto">
              {loading ? (
                <div className="flex items-center justify-center h-40 text-[var(--muted-foreground)] text-sm">Loading…</div>
              ) : messages.length === 0 ? (
                ...
                messages.map(msg => (
                  <button
                    ...
                    className={`w-full text-left px-4 py-3 border-b border-[var(--border)] transition-colors ${
                      selected === msg.id ? 'bg-blue-50 border-l-2 border-l-primary' : 'hover:bg-gray-50'
                    }`}
                  >
                    <div className="flex items-center justify-between mb-0.5">
                      <span className={`text-xs font-semibold ${msg.read ? 'text-[var(--muted-foreground)]' : 'text-[var(--foreground)]'}`}>
                        {msg.from === 'Admin' ? '🔵 Admin' : '↗ You'}
                      </span>
                      <span className="text-[10px] text-[var(--muted-foreground)]">{msg.time}</span>
                    </div>
                    <p className={`text-xs truncate mb-0.5 ${msg.read ? 'font-normal text-[var(--muted-foreground)]' : 'font-bold text-[var(--foreground)]'}`}>
                      {msg.subject}
                    </p>
                    <p className="text-[10px] text-[var(--muted-foreground)] truncate">{msg.text}</p>
                    {!msg.read && (
            ...
            {composing ? (
              <>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-4">
                  <div>
                    <label className="block text-xs font-semibold text-[var(--muted-foreground)] uppercase mb-1">Subject</label>
                    <input
                      value={newSubject}
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-[var(--muted-foreground)] uppercase mb-1">Linked Order ID (optional)</label>
                    <input
                      value={newOrderId}
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-[var(--muted-foreground)] uppercase mb-2">Quick Templates</label>
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-[var(--muted-foreground)] uppercase mb-1">Message</label>
                    <textarea
                      value={newBody}
              ...
              <>
                ...
                <div className="px-6 py-4 border-b border-[var(--border)]">
                  ...
                  <div className="flex items-center gap-4 text-xs text-[var(--muted-foreground)]">
                    <span>From: <strong>{selectedMsg.from === 'Admin' ? '🔵 Admin' : 'You'}</strong></span>
                    <span>To: <strong>{selectedMsg.to === 'Admin' ? '🔵 Admin' : 'You'}</strong></span>
                    <span className="flex items-center gap-1"><Clock size={10} /> {selectedMsg.time}</span>
                    {selectedMsg.orderId && (
                      <span className="px-2 py-0.5 bg-gray-100 rounded font-mono text-[10px]">{selectedMsg.orderId}</span>
                    )}
                  </div>
                </div>
                <div className="flex-1 overflow-y-auto px-6 py-5">
                  <p className="text-sm leading-relaxed text-[var(--foreground)] whitespace-pre-wrap">{selectedMsg.text}</p>
                  {selectedMsg.from === 'Admin' && (
                    <div className="mt-4 p-3 bg-blue-50 rounded-lg border border-blue-100">
                      <p className="text-[10px] font-semibold text-blue-600 uppercase mb-1">Note</p>
                      <p className="text-xs text-blue-700">This message was sent by the Proquoment Admin team. All replies go directly to Admin.</p>
                    </div>
                  )}
                </div>
                {/* Reply Box */}
                <div className="px-6 py-4 border-t border-[var(--border)]">
                  <div className="mb-3 flex flex-wrap gap-2">
                    {QUICK_TEMPLATES.map(t => (
                      <button
                        key={t.label}
                        onClick={() => { setReply(t.text); replyRef.current?.focus(); }}
                        className="px-2 py-1 text-[10px] font-semibold border border-[var(--border)] rounded-lg hover:bg-gray-50 transition-colors"
                      >
                        {t.label}
                      </button>
                  ...
                  <p className="text-[10px] text-[var(--muted-foreground)] mt-1">Ctrl+Enter to send</p>
                </div>
              </>
            ) : (
