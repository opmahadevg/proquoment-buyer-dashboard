...
export default function UpdatesTab({ tasks, updates }: UpdatesTabProps) {
  ...
  return (
    <div>
      ...
      {/* Tasks section */}
      <h3 className="text-base font-semibold text-[var(--foreground)] mb-3">{tasks?.length} Task{tasks?.length !== 1 ? 's' : ''}</h3>
      {tasks?.map((task) => (
        <div key={task?.id} className="border border-[var(--border)] rounded-xl px-5 py-4 mb-4 hover:bg-[var(--muted)]/20 transition-colors cursor-pointer">
      ...
      <div className="flex items-center justify-between mb-4 mt-6">
        <h3 className="text-base font-semibold text-[var(--foreground)]">{updates?.length} Update{updates?.length !== 1 ? 's' : ''}</h3>
      ...
      {updates?.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 gap-4">
          ...
          <div className="text-center">
            <p className="text-sm font-semibold text-[var(--foreground)] mb-1">No unresolved updates</p>
            <p className="text-sm text-[var(--muted-foreground)]">There are no unresolved updates for this product</p>
          </div>
        </div>
      ) : (
        updates?.map((upd) => (
          <div key={upd?.id} className="border border-[var(--border)] rounded-xl px-5 py-4 mb-4 hover:bg-[var(--muted)]/20 transition-colors cursor-pointer">
            <p className="text-sm font-semibold text-[var(--foreground)] mb-1">{upd?.title}</p>
            <p className="text-sm text-[var(--muted-foreground)] mb-3 leading-relaxed">{upd?.description}</p>
            <div className="flex items-center justify-between text-xs text-[var(--muted-foreground)]">
              <div className="flex items-center gap-3">
                <span>{upd?.date}</span>
                <span className="flex items-center gap-1">
